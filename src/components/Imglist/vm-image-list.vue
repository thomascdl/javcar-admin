<template>
  <div class="vm-image-list">
    <Row class="image-list-heading vm-panel">
      ...
      <Row type="flex" align="middle" justify="space-between" class="panel-body">
        <div class="search-bar">
          <Input v-model="listQuery.fh" placeholder="Please enter ..." @keyup.enter.native="search"
        ...
        <Row type="flex" align="middle" class="page">
          <span>Show</span>
          <Input v-model="listQuery.limit" :max="42" :min="10" :number="true" class="input-number"
                 @keyup.enter.native="updateDataShow"/></Input>
    ...
    <Row class="image-list" :gutter="16">
      <Col :lg="4" :sm="12" class="vm-margin" v-for="item in data" :key="item.id">
        <VmCard :editable="true" :title="item.name" :img="item.url" @delete-ok="deleteOk(item)"></VmCard>
      </Col>
    </Row>
